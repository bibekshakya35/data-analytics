        ...
        Testing.withSimulatedTimeLocalCluster(clusterParam, new TestJob() {
            ...
            public void run(ILocalCluster cluster) throws Exception {
                ...
                assertTrue(Testing.multiseteq(new Values(
                        new Values("12345 test@manning.com")
                ),Testing.readTuples(result,"commit-feed-listener")));
                assertTrue(
                        Testing.multiseteq(new Values(
                                new Values("test@manning.com")),
                        Testing.readTuples(result,"email-extractor")
                ));
